...
app.get('/api/download/:productId/:buyerAddress', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Download sanitize error:', { productId, buyerAddress, err: error.message });
      return res.status(400).json({ error: 'Invalid address format', productId, buyerAddress });
    }

    // Verify purchase — use LOWER() for case-insensitive match
    const purchase = await pool.query(
      'SELECT id FROM purchases WHERE LOWER(product_id) = LOWER($1) AND LOWER(buyer) = LOWER($2)',
      [productId, buyerAddress]
    );
    ...
    // Get file info — use LOWER() for case-insensitive match
    const product = await pool.query(
      'SELECT file_cid, file_name FROM products WHERE LOWER(id) = LOWER($1)',
      [productId]
    );
...
// ==================== License Endpoints (Feature Branch) ====================

// GET /api/licenses/:address — all licenses for a buyer
app.get('/api/licenses/:address', async (req: any, res: any) => {
  try {
    const { address } = req.params;
    ...
    // Enrich each license with its activations
    const licenses = await Promise.all(result.rows.map(async (lic: any) => {
      const activations = await pool.query(
        `SELECT device_id, activated_at, is_active
         FROM license_activations
         WHERE license_id = $1 AND is_active = true`,
        [lic.license_id]
      );
      return { ...lic, active_devices: activations.rows };
    }));

    res.json({ licenses });
  } catch (error: any) {
    res.status(500).json({ error: 'Failed to fetch licenses', detail: error.message });
  }
});

// GET /api/licenses/verify/:licenseId — verify a license (for sellers/devs)
app.get('/api/licenses/verify/:licenseId', async (req: any, res: any) => {
  try {
    ...
    const now     = Date.now();
    const expired = Number(license.expiry_timestamp) !== 0
      && Number(license.expiry_timestamp) < now;
    ...
    res.status(500).json({ error: 'Failed to verify license', detail: error.message });
  }
});

// GET /api/licenses/:licenseId/activations — list active devices
app.get('/api/licenses/:licenseId/activations', async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT device_id, activated_at, is_active
       FROM license_activations
    ...
    res.status(500).json({ error: 'Failed to fetch activations', detail: error.message });
  }
});

// GET /api/licenses/:licenseId/renewals — renewal history
app.get('/api/licenses/:licenseId/renewals', async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT * FROM license_renewals
       WHERE license_id = $1
       ORDER BY renewal_number ASC`,
      [req.params.licenseId]
    );
    res.json({ renewals: result.rows });
  } catch (error: any) {
    res.status(500).json({ error: 'Failed to fetch renewals', detail: error.message });
  }
});

// GET /api/seller/licenses/:address — seller sees all licenses for their products
app.get('/api/seller/licenses/:address', async (req: any, res: any) => {
  try {
    const result = await pool.query(
      `SELECT l.*, p.title AS product_title
       FROM licenses l
    ...
    res.status(500).json({ error: 'Failed to fetch seller licenses', detail: error.message });
  }
});
